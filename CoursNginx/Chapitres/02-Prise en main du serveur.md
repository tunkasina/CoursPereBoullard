...
<li>vous devez mettre la clé publique dans `.ssh/authorized_keys`, si le répertoire n'existe pas, créez-le avec les droits **700**.</li>
<li>Puis `cat [NOM_EXPLICITE].pub >> .ssh/authorized_keys` - et si le fichier n'existait pas, donnez-lui les droits **600**.</li>
</ul>
...
<li>Copier : `scp webadmin@[VOTRE_IP]:/home/webadmin/[NOM_EXPLICITE] ./.ssh/`</li>
</ul>
<p>Ensuite on configure le fichier de conf du démon :</p>
<ul>
<li>`/etc/ssh/sshd_config` (il faut dé-commenter `PublickeyAuthentication yes` et `AuthorizedKeysFile .ssh/authorized_keys`)</li>
