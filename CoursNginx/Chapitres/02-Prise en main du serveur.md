...
Côté serveur, basculez sur un prompt en tant que _webadmin_, et :
 ...
 - Puis `cat [NOM_EXPLICITE].pub >> .ssh/authorized_keys` - et si le fichier n'existait pas, donnez-lui les droits **600**.
...
Côté client, pour éviter les soucis d'encodage, on copie le fichier depuis notre poste en le prenant sur le serveur :
 ...
 - Copier : `scp webadmin@[VOTRE_IP]:/home/webadmin/[NOM_EXPLICITE] ./.ssh/`

Ensuite on configure le fichier de conf du démon :
 - `/etc/ssh/sshd_config` (il faut dé-commenter `PublickeyAuthentication yes` et `AuthorizedKeysFile .ssh/authorized_keys`)
